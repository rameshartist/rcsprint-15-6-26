...
final class ComboOfferManager
{
    ...
    public static function all(bool $public = false): array
    {
        self::ensureSchema();
        $where = $public ? "WHERE c.is_active=1" : "";
        return \Database::rows("SELECT c.*,(SELECT COUNT(*) FROM combo_offer_items ci WHERE ci.combo_offer_id=c.id)+(SELECT COUNT(*) FROM combo_offer_custom_items cc WHERE cc.combo_offer_id=c.id) item_count FROM combo_offers c $where ORDER BY c.sort_order,c.id DESC");
    }
    ...
    {
        $products = \Database::rows("SELECT p.id,p.name,p.slug,(SELECT COALESCE(pi.image_path,pi.url) FROM product_images pi WHERE pi.product_id=p.id ORDER BY pi.is_primary DESC,pi.id LIMIT 1) thumbnail FROM products p WHERE p.is_active=1 ORDER BY p.name");
    ...
    public static function save(array $data, int $id = 0): array
    {
        ...
        foreach ((array)($data['custom_items'] ?? []) as $row) {
            $name = trim((string)($row['item_name'] ?? '')); $price = round(max(0, (float)($row['item_price'] ?? 0)), 2);
            if ($name === '' && $price <= 0) continue;
            if ($name === '' || $price <= 0) return ['ok'=>false,'msg'=>'Every custom item needs a name and price.'];
            $customItems[] = ['item_name'=>$name,'item_price'=>$price,'thumbnail_path'=>trim((string)($row['thumbnail_path'] ?? ''))];
            $subtotal += $price;
        }
        ...
        $values = [$title,$slug,trim((string)($data['badge']??'')),trim((string)($data['short_description']??'')),trim((string)($data['description']??'')),trim((string)($data['banner_image']??'')),$subtotal,$discount,$offerPrice,in_array(($data['layout_slot']??''),['large','wide','square'],true)?$data['layout_slot']:'square',trim((string)($data['cta_text']??'View Offer'))?:'View Offer',(int)($data['sort_order']??0),!empty($data['show_on_home'])?1:0,!empty($data['is_active'])?1:0];
        $db = \Database::get();
        try {
            $db->beginTransaction();
            if ($id) {
                \Database::query("UPDATE combo_offers SET title=?,slug=?,badge=?,short_description=?,description=?,banner_image=?,regular_price=?,discount_percent=?,combo_price=?,layout_slot=?,cta_text=?,sort_order=?,show_on_home=?,is_active=?,updated_at=NOW() WHERE id=?", [...$values,$id]);
            } else {
                $id = \Database::insert("INSERT INTO combo_offers(title,slug,badge,short_description,description,banner_image,regular_price,discount_percent,combo_price,layout_slot,cta_text,sort_order,show_on_home,is_active,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())", $values);
            }
            ...
            foreach ($customItems as $i => $row) \Database::insert("INSERT INTO combo_offer_custom_items(combo_offer_id,item_name,item_price,thumbnail_path,sort_order,created_at) VALUES(?,?,?,?,?,NOW())", [$id,$row['item_name'],$row['item_price'],$row['thumbnail_path'],$i]);
            $db->commit();
        } catch (\Throwable $e) {
