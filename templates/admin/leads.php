...
window.addEventListener('admin:live-updates',e=>{const next=Number(e.detail.counts.leads||0),prev=Number(e.detail.previous.leads||0);if(next!==prev&&!document.getElementById('leadDrawer')?.classList.contains('open'))loadLeads();});
loadLeadTemplates().then(loadLeads).then(()=>{const match=location.hash.match(/^#lead-(\d+)$/);if(match)openLead(Number(match[1]));});
</script>
