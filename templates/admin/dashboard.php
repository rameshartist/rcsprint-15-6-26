...
async function loadDash(){const updated=document.getElementById('dashUpdated');if(updated)updated.textContent='Refreshing live data…';try{const response=await fetch('/admin/api/dashboard',{credentials:'same-origin',cache:'no-store'}),res=await response.json();if(!response.ok||!res.ok)throw new Error(res.msg||'Dashboard request failed');const s=res.stats||{};document.getElementById('ds-total-orders').textContent=Number(s.total_orders||0).toLocaleString('en-IN');document.getElementById('ds-today-rev').textContent=money(s.today_revenue);document.getElementById('ds-month-rev').textContent=money(s.month_revenue);document.getElementById('ds-rev').textContent=money(s.total_revenue);document.getElementById('ds-customers').textContent=Number(s.total_customers||0).toLocaleString('en-IN');setTrend('tr-today-rev',s.today_revenue_trend,s.today_revenue_trend_label);setTrend('tr-month-rev',s.month_revenue_trend,s.month_revenue_trend_label);setTrend('tr-customers',s.total_customers_trend,s.total_customers_trend_label);drawRevenue(res.monthly||[]);drawQueue(res.queue||{});drawProducts(res.top_products||[]);drawOrders(res.recent_orders||[]);drawCustomers(res.recent_customers||[]);if(updated)updated.textContent='Updated '+new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});}catch(error){if(updated)updated.textContent='Dashboard data is temporarily unavailable.';}}
